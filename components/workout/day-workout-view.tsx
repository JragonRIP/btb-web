...
export function DayWorkoutView({ dow }: { dow: number }) {
  ...
  const allChecked =
    exercises.length > 0 && exercises.every((ex) => checked[ex.id]);
    ...
    <div className={cn(celebrate && "animate-pulse")}>
      ...
      <div className="relative mx-auto max-w-3xl space-y-4 px-4 py-6">
        ...
        {!plan || exercises.length === 0 ? (
          <Card className="p-6 text-sm text-muted">No exercises for this day. Edit your weekly plan in Settings.</Card>
        ) : (
          exercises.map((ex) => (
